...
public class AdTestActivity extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.lay_adtest);
    ...
    private void setHuvleAD() {
        ...
        */
        ...
        bav = findViewById(R.id.adview_container);
        initBannerView(bav, "testbig",320,50); // 300 * 250 배너 테스트시(Example for testing 300 * 250 banner)  initBannerView(staticBav, "testbig",300,250);

        // 동적으로 구현시(When if apply Static Implementation) BannerAdView Start

//        final BannerAdView bav = new BannerAdView(this);
//        initBannerView(bav, "testbig",300,250); // 300 * 250 배너 테스트시 initBannerView(staticBav, "testbig",300,250);
//        RelativeLayout layout = (RelativeLayout) findViewById(R.id.adview_container);
...
//        bav.setResizeAdToFitContainer(true);
        ...
        AdListener adListener = new AdListener() {
            @Override public void onAdRequestFailed(AdView bav, ResultCode errorCode) {/*광고가 없을때 처리(Handle when there is no advertiment)*/
                if (errorCode == null) {
                    Clog.v("Huvle_Banner", "Call to loadAd failed");
                } else {
                    Clog.v("Huvle_Banner", "Ad request failed: " + errorCode);
                }}
